...
#####################################################
    ...
    #
    if (rx_time == None):
        fp = open(outfile, "wb") #modo binario 
    ...
    #
    # The rest here is mostly due to Guillermo Gancio ganciogm@gmail.com and fixed by Oscar Zamora (for Python 3)
    #
    ...
    fp.flush()
    #--
    aux="rawdatafile".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    fp.write(struct.pack('i', len(outfile.encode("utf-8")))+outfile.encode("utf-8"))#outfile is str---> decode to byte in order to get (byte+byte) 
    #--
    aux="src_raj".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    ...
    fp.write(aux)
    #--
    aux="az_start".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', 0.0)
    fp.write(aux)
    #--
    aux="za_start".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', 0.0)
    fp.write(aux)
    #--
    aux="tstart".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', float(t_start))
    fp.write(aux)
    #--
    aux="foff".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', f_off)
    fp.write(aux)
    #--
    aux="fch1".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', high_freq)
    fp.write(aux)
    #--
    aux="nchans".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', sub_bands)
    fp.write(aux)
    #--
    aux="data_type".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 1)
    fp.write(aux)
    #--
    aux="ibeam".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 1)
    fp.write(aux)
    #--
    aux="nbits".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', smpsize*8)
    fp.write(aux)
    #--
    aux="tsamp".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('d', tsamp)
    fp.write(aux)
    #--
    aux="nbeams".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 1)
    fp.write(aux)
    #--
    aux="nifs".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 1)
    fp.write(aux)
    #--
    aux="source_name".encode("utf-8")
    fp.write(struct.pack('i', len(aux))+aux)
    fp.write(struct.pack('i', len(source_name.encode("utf-8")))+source_name.encode("utf-8"))#source_name is str---> decode to byte 
    #--
    aux="machine_id".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 20)
    fp.write(aux)
    #--
    aux="telescope_id".encode("utf-8")
    aux=struct.pack('i', len(aux))+aux
    fp.write(aux)
    aux=struct.pack('i', 20)
    fp.write(aux)
    ...
    #fp.flush() is not working here 
    #fp.close   is not working here 
    return True
   
def auto_to_freqs(mask, freq, bw): #bw is float 
    start = freq+(bw/2)
    bins = len(mask)
    decr = bw/bins
    flist = ""
...
#  update code
#
""" only used in debuging 
LOG_FILE = "/home/rt/pulsar_filterbank/logs/tags.txt" 
...
def process_tag(tags):
    ...
    for tag in tags:
        ...
        tag_dict[key] = value
        #log_message(f"Tag recibido: {key} = {value}") only for debug 

        if first_tag is None:
            first_tag = time.time()
            #log_message(f" first_tag registrado: {first_tag}") only for debug 
...
#
""" only used in debuging
LOG_FILEe = "/home/rt/pulsar_filterbank/logs/update_header.txt"

def log_message2(message2):
    with open(LOG_FILEe, "a") as log:
        log.write(message2 + "\n")
"""
def update_header(pacer, runtime, smpsize):
    ...
    #log_message2(f"Estado de didit antes de procesar: {didit}")

    if runtime is not None:
        if first_tag is None:
            #log_message2(" No hay first_tag. No se puede actualizar la cabecera.")
            return None
        else:
            endtime = first_tag + runtime - 0.5
    else:
        endtime = time.time() - 30.0

    if (time.time() >= endtime) and not didit:
        #log_message2("Actualizando la cabecera y concatenando datos...")
        ...
        if times is not None:
            ...
            seconds = first_tag if first_tag is not None else time.time()
            #log_message2("No se encontró rx_time, usando tiempo aproximado.")
        MJD = seconds / 86400.0 + 40587.0
        ...
        try:
            ...
            #
            os.remove(dataname)###line anull debuging
            #log_message2(f" bytes concatenados correctamente en {header_args['outfile']} ({os.path.getsize(header_args['outfile'])} bytes)")
            didit = True  
...
def dynamic_mask(fft,smask,thresh):
    ...
    if ((count % 15) == 0):
        ...
        #
        mcount = len(trim_fft)-trim_mask.count(0.0) #es int
